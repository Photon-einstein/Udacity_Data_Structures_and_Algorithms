...
def dijkstra(graph, start_node, end_node):
    ...
    while distance_dict:
        ...
        # Check for each neighbor of current_node, if the distance_to_neighbor is smaller than the already stored distance, update the distance_dict
        for edge in current_node.edges:
            if edge.node in distance_dict:
                
                distance_to_neighbor = node_distance + edge.distance
                if distance_dict[edge.node] > distance_to_neighbor:
                    distance_dict[edge.node] = distance_to_neighbor
